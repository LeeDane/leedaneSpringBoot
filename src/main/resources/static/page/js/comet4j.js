...
JS.Runtime = (function() {
	var ua = navigator.userAgent.toLowerCase(), check = function(r) {
		return r.test(ua);
	}, isOpera = check(/opera/), isFirefox = check(/firefox/), isChrome = check(/chrome/), isWebKit = check(/webkit/), isSafari = !isChrome
			...
			&& check(/rv:1\.9/), isWindows = check(/windows|win32/), isMac = check(/macintosh|mac os x/), isAir = check(/adobeair/), isLinux = check(/linux/);
	return {
		isOpera : isOpera,
		isFirefox : isFirefox,
		isChrome : isChrome,
...
JS.Observable.prototype = {
	...
	fireEvent : function() {
		var arg = JS.toArray(arguments), eventName = arg[0].toLowerCase(), e = this.events[eventName];
		if (e && !JS.isBoolean(e)) {
	...
	hasListener : function(eventName, fn, scope) {
		var events = this.events, e = events[eventName];
		if (!JS.isBoolean(e)) {
...
JS.Event.prototype = {
	fire : function() {
		var listeners = this.listeners, i = listeners.length - 1;
		for (; i > -1; i--) {
	...
	hasListener : function(fn, scope) {
		var i = 0, listeners = this.listeners, len = listeners.length;
		for (; i < len; i++) {
...
JS.XMLHttpRequest = JS.extend(JS.Observable, {
	...
	constructor : function() {
		var self = this;
		this.addEvents([ 'readyStateChange', 'timeout', 'abort', 'error',
				'load', 'progress' ]);
	...
	createXmlHttpRequestObject : function() {
		var activeX = [ 'Msxml2.XMLHTTP.6.0', 'Msxml2.XMLHTTP.5.0',
				...
				'Microsoft.XMLHTTP' ], xhr, specialXHR = this.specialXHR;
		if (specialXHR) {
	...
	doReadyStateChange : function() {
		...
		this.fireEvent('readyStateChange', this.readyState, this.status, this,
				xhr);
...
JS.Connector = JS
		.extend(
				JS.Observable,
				{
					...
					lastReceiveMessage : '',
					constructor : function() {
						JS.Connector.superclass.constructor.apply(this,
								arguments);
						this.addEvents([ 'beforeConnect', 'connect',
								'beforeStop', 'stop', 'message', 'revival' ]);
						if (JS.isIE7) {
						...
						this._xhr.addListener('progress', this.doOnProgress,
								this);
						this._xhr.addListener('load', this.doOnLoad, this);
						this._xhr.addListener('error', this.doOnError, this);
						this._xhr.addListener('timeout', this.revivalConnect,
								this);
					...
					doDrop : function(url, cId, conn, xhr) {
						...
						try {
							...
							var url = parseUrl(this.url) + this.CMDTAG
									+ '=drop&cid=' + this.cId;
					...
					dispatchServerEvent : function(msg) {
						this.fireEvent('message', msg.channel, msg.data,
								msg.time, this);
					},
					...
					doOnProgress : function(xhr) {
						if (this.workStyle === this.STREAMSTYLE) {
							var str = this
									.translateStreamData(xhr.responseText);
					...
					startConnect : function() {
						if (this.running) {
							var url = parseUrl(this.url) + this.CMDTAG
									+ '=conn&cv=' + this.version + this.param;
							JS.AJAX.get(url, '', function(xhr) {
								...
								this.fireEvent('connect', data.cId,
										data.channels, data.ws, data.timeout,
										this);
								this.revivalConnect();
							}, this);
						}
					...
					revivalConnect : function() {
						...
						function revival() {
							...
							/**
							 * 对返回的数据进行处理(由于没有处理这步的url，导致获取不到后台返回的数据)
							 */
							var url = parseUrl(self.url) + self.CMDTAG
									+ '=revival&cid=' + self.cId + self.param;
					...
					start : function(url, param) {
						...
						setTimeout(
								function() {
									...
									if (self.fireEvent('beforeConnect',
											self.url, self) === false) {
										return;
									}
									self.running = true;
					...
					stop : function(cause) {
						...
						if (this.fireEvent('beforeStop', cause, this.cId,
								this.url, this) === false) {
							return;
						}
						this.running = false;
