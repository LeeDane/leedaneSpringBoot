...
public class LuceneTest extends BaseTest{
	...
	public void searchTest() throws IOException, ParseException{
		...
		/*try {
			...
			logger.info("命中：" + topDocs.totalHits); //输出结果 
			...
			for (int i = 0; i < topDocs.totalHits; i++){ 
				...
				for(IndexableField field: targetDoc.getFields()){
					...
					if( name!= null && name.equals("bookcontent")){
						logger.info("内容：" + field.stringValue()); 
					}
				},"FB1724673
		...
		logger.info("总数：" +LuceneUtil.getInstance().simpleSearch(new String[]{"id"}, "327", 50));
		long endTime = System.currentTimeMillis();
		logger.info("执行程序总计耗时："+ (endTime - startTime) + "毫秒");
	}
	...
	public void updateDocument() throws IOException, ParseException{
		...
		logger.info(LuceneUtil.getInstance().updateIndex("booktitle", "17277233", bookdoc));
	}
	...
	public void indexSanwen() throws IOException, ParseException{
		...
		if(root != null && root.isDirectory()){
			...
			for (int i = 0; i < fs.length; i++) {
				...
				//logger.info("f_originPath:" + f_originPath);
				...
				logger.info("加载文档"+ i + "-----"+ f_originPath + "---->索引成功");
			 }  
	...
	public void searchSanwen() throws IOException, ParseException{
		long startTime = System.currentTimeMillis();
		logger.info("总数：" +LuceneUtil.getInstance().simpleSearch(new String[]{"title","content"}, "消息", 50));
		long endTime = System.currentTimeMillis();
		logger.info("执行程序总计耗时："+ (endTime - startTime) + "毫秒");
	}
	...
	public void searchSanwenPageByAfter() throws IOException, ParseException{
		long startTime = System.currentTimeMillis();
		logger.info("总数：" +LuceneUtil.getInstance().simpleSearchPageByAfter(new String[]{"booktitle","bookcontent"}, "我", 1, 15));
		long endTime = System.currentTimeMillis();
		logger.info("执行程序总计耗时："+ (endTime - startTime) + "毫秒");
	}
