...
public class OrderController extends BaseController{
	...
	@Autowired
	private S_OrderService<S_OrderBean> orderService;
	...
	public ResponseModel add(Model model, HttpServletRequest request) throws Exception {
		...
		return orderService.add(getJsonFromMessage(message), getUserFromMessage(message), getHttpRequestInfo(request));
	}
	...
	public ResponseModel update(@PathVariable("orderId") long orderId, Model model, HttpServletRequest request) throws Exception {
		ResponseMap message = new ResponseMap();
		if(!checkParams(message, request))
			return message.getModel();
		checkRoleOrPermission(model, request);
		return orderService.update(orderId, getJsonFromMessage(message), getUserFromMessage(message), getHttpRequestInfo(request));
	}
	...
	public ResponseModel delete(@PathVariable("orderId") long orderId, Model model, HttpServletRequest request){
		ResponseMap message = new ResponseMap();
		if(!checkParams(message, request))
			return message.getModel();
		checkRoleOrPermission(model, request);
		return orderService.delete(orderId, getUserFromMessage(message), getHttpRequestInfo(request));
	}
	...
	public ResponseModel getNumber(Model model, HttpServletRequest request){
		ResponseMap message = new ResponseMap();
		if(!checkParams(message, request))
			return message.getModel();
		checkRoleOrPermission(model, request);
		return orderService.getNoDealOrderNumber(getUserFromMessage(message), getHttpRequestInfo(request));
	}
	...
	public LayuiTableResponseModel mywish(Model model, HttpServletRequest request,
...
//		LayuiTableResponseModel responseModel = new LayuiTableResponseModel(0, "", 0, null);
		checkParams(message, request);
		checkRoleOrPermission(model, request);
		return orderService.paging(current - 1, pageSize, getUserFromMessage(message), getHttpRequestInfo(request));
	}
}
