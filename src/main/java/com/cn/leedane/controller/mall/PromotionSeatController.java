...
public class PromotionSeatController extends BaseController{
	 ...
	 * @return
	 */
	@RequestMapping(value = "/{promotionSeatId}", method = RequestMethod.PUT, produces = {"application/json;charset=UTF-8"})
	public Map<String, Object> update(@PathVariable("promotionSeatId") long promotionSeatId, Model model, HttpServletRequest request) throws Exception {
		ResponseMap message = new ResponseMap();
		if(!checkParams(message, request))
			return message.getMap();

		checkRoleOrPermission(model, request);;
		message.putAll(promotionSeatService.update(promotionSeatId, getJsonFromMessage(message), getUserFromMessage(message), getHttpRequestInfo(request)));
		return message.getMap();
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/{promotionSeatId}", method = RequestMethod.DELETE, produces = {"application/json;charset=UTF-8"})
	public Map<String, Object> delete(@PathVariable("promotionSeatId") long promotionSeatId, Model model, HttpServletRequest request){
		ResponseMap message = new ResponseMap();
		if(!checkParams(message, request))
			return message.getMap();

		checkRoleOrPermission(model, request);;
		message.putAll(promotionSeatService.delete(promotionSeatId, getUserFromMessage(message), getHttpRequestInfo(request)));
		return message.getMap();
	}
	...
	public Map<String, Object> mywish(Model model, HttpServletRequest request,
		...
		checkRoleOrPermission(model, request);;
		message.putAll(promotionSeatService.paging(current - 1, pageSize, getJsonFromMessage(message), getUserFromMessage(message), getHttpRequestInfo(request)));
		return message.getMap();
	}
}
