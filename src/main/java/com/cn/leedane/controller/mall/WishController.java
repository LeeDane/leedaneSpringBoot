...
public class WishController extends BaseController{
	...
	public ResponseModel add(Model model, HttpServletRequest request){
		...
		return wishService.add(getJsonFromMessage(message), getUserFromMessage(message), getHttpRequestInfo(request));
	}
	...
	public ResponseModel delete(@PathVariable("wishId") long wishId, Model model, HttpServletRequest request){
		ResponseMap message = new ResponseMap();
		if(!checkParams(message, request))
			return message.getModel();
		checkRoleOrPermission(model, request);
		return wishService.delete(wishId, getUserFromMessage(message), getHttpRequestInfo(request));
	}
	...
	public ResponseModel getNumber(Model model, HttpServletRequest request){
		...
		if(!checkParams(message, request))
			return message.getModel();
		checkRoleOrPermission(model, request);
		return wishService.getWishNumber(getUserFromMessage(message), getHttpRequestInfo(request));
	}
...
//		LayuiTableResponseModel message = new LayuiTableResponseModel(0, "", 0, null);
		ResponseMap message = new ResponseMap();
		
		checkRoleOrPermission(model, request);;
		return wishService.paging(current - 1, pageSize, getUserFromMessage(message), getHttpRequestInfo(request));
	}
}
