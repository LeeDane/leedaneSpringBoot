...
public class StockBuyServiceImpl implements StockBuyService<StockBuyBean>{
	...
	public Map<String, Object> add(int stockId, JSONObject json, UserBean user,
		...
		//保存操作日志
		operateLogService.saveOperateLog(user, request, null, StringUtil.getStringBufferStr(user.getAccount(),"添加股票购买记录，ID为", stockBuyBean.getId(), "，结果是：", StringUtil.getSuccessOrNoStr(result)).toString(), "add()", ConstantsUtil.STATUS_NORMAL, EnumUtil.LogOperateType.内部接口.value);
