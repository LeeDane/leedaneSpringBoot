 ...
 * 商店商品solr处理类
 * @author LeeDane
 * 2017年11月15日 下午5:08:08
 * version 1.0
 */
public class ProductSolrHandler implements BaseSolrHandler<S_ProductBean> {
public volatile static ProductSolrHandler handler;
	
	public static HttpSolrServer server;
	
	private ProductSolrHandler() {
	...
	public synchronized static ProductSolrHandler getInstance(){
		if(handler == null){
			synchronized(ProductSolrHandler.class){
				if(handler == null)
					handler = new ProductSolrHandler();
			}
		}
		return handler;
	}
...
//					QueryResponse response = server.query(params);
	...
	@Override
	public boolean addBean(S_ProductBean bean){
		try {
			server.addBean(bean);
			server.commit();
			return true;
	...
	@Override
	public boolean addBeans(List<S_ProductBean> beans){
		try {
	...
	@Override
	public boolean updateBean(S_ProductBean bean) {
		return addBean(bean);
	}

	@Override
	public boolean updateBeans(List<S_ProductBean> beans) {
