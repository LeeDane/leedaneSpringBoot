	       	...
	       	if(CollectionUtil.isNotEmpty(userTokenBeans)){
	       		for(UserTokenBean userTokenBean: userTokenBeans){
	       			if(userTokenBean.getToken().equals(customAuthenticationToken.getToken()))
	       				return new SimpleAuthenticationInfo(customAuthenticationToken, userTokenBean.getToken(), getName());
	       		}
	       	}
	       	
	       	throw new UnsupportedTokenException(); //抛出不支持的token异常	
        }if(customAuthenticationToken.getPlatformType() == PlatformType.微信公众平台版){
	       		return new SimpleAuthenticationInfo(customAuthenticationToken, customAuthenticationToken.getPassword(), getName());
        }else{
        	return new SimpleAuthenticationInfo(customAuthenticationToken, customAuthenticationToken.getPassword(), getName());
        }
    }
