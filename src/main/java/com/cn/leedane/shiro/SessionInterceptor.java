...
//    private PermissionService permissionService;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if(!currentUser.isAuthenticated() && currentUser.isRemembered()){
            try {
                CustomAuthenticationToken customAuthenticationToken = (CustomAuthenticationToken)currentUser.getPrincipal();
                Session session = SecurityUtils.getSubject().getSession();
                session.setAttribute("remember", true);
                session.setTimeout(-1001L);
                SessionManagerUtil.getInstance().addSession(currentUser, currentUser.getSession(), customAuthenticationToken.getUser().getId());
